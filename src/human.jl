...
pedestrian_potential_function(barrier, coeff) =  coeff * exp(-barrier)
...
function social_forces(human, state, goal_position, others, desired_velocity; p=2., τ=2., ψ=pi/6, c=0.3)
    dt = human.dt
    # goal force
    position = get_position(human, state)
    velocity = get_velocity(human, state, [0.; 0.])
    ...
    for (dyn_o, o_state) in others
        ...
        o_speed = get_speed(dyn_o, o_state, [0.; 0.])
        V_b_x(x) = pedestrian_potential_function(barrier(x - o_position, o_speed, o_position, dt), p)
        ped_force = -ForwardDiff.gradient(V_b_x, position)
        fov_weight = field_of_view(ψ, c, position, -ped_force)
        force += fov_weight * ped_force
    end
    if norm(force) > norm(human.control_max)
        force /= norm(force) * norm(human.control_max)
    end
    force
end
...
state = [1.; 0.; 1.; 0.]
goal_position = [10.; 1.]
others = [[human, [2; .0; -.0; 0.]]]
desired_velocity = 2.
forces = social_forces(human, state, goal_position, others, desired_velocity, p=2., ψ=pi/6, c=0.3)
